...
typedef ConfigData =
{
	timeBarColor1:Dynamic,
	timeBarColor2:Dynamic
}
...
class PlayState extends MusicBeatState
{
	...
	public var notCheating:Bool = false;
	...
	override public function create()
	{
		...
		openfl.Lib.application.window.title = Main.appTitle 
		+ ' - ' 
		+ SONG.song;
		...
		configJSON = Json.parse(Paths.getTextFromFile('data/' + songName + '/config.json'));
		...
		if(SONG.stage == null || SONG.stage.length < 1) {
			switch (songName)
			{
				...
				case 'flippin out':
					...
					if(FlxG.random.bool(1.15)) curStage = 'land-bloody';
				case 'unflipped out':
					curStage = 'land-lol';
					if(FlxG.random.bool(1.15)) curStage = 'land-bloody-day';
				default:
					curStage = 'stage';
			}
		}
		...
		switch (curStage)
		{
			...
			case 'land-cute': //Week 1
				var bg:BGSprite = new BGSprite('$curStage/bgWalls', -1000, -500, 0.2, 0.2);
				bg.setGraphicSize(Std.int(bg.width * 0.8));
				bg.updateHitbox();
				add(bg);

				var fgSnow:BGSprite = new BGSprite('$curStage/fgSnow', -600, 700);
				add(fgSnow);
			...
			case 'land-destroyed': //Week 1
				var bg:BGSprite = new BGSprite('$curStage/stageback', -600, -200, 0.9, 0.9);
				bg.updateHitbox();
				add(bg);

				var stageFront:BGSprite = new BGSprite('$curStage/stagefront', -650, 550);
				stageFront.setGraphicSize(Std.int(stageFront.width * 1.1));
				stageFront.updateHitbox();
				add(stageFront);

			case 'land-destroyed-old': //Week 1
				var bg:BGSprite = new BGSprite('$curStage/stageback', -600, -200, 0.9, 0.9);
				add(bg);
		...
		if(gfVersion == null || gfVersion.length < 1)
		{
			switch (curStage)
			{
				case 'land-cute' | 'land-deadbodys':
					gfVersion = 'gf-statue';
				case 'land-destroyed' | 'land-bloody':
					gfVersion = 'gf-hurt';
				default:
					gfVersion = 'gf';
			}
		...
		if (stageData.isCreepyStage)
		{
			GameOverSubstate.characterName = 'bf-creepy-dead';
		...
		if(curStage == 'land-cute')
			timeBar.createFilledBar(0xFF000000, 0xFF00FFEA);
		else if(curStage == 'land-deadbodys')
			timeBar.createFilledBar(0xFF000000, 0xFF880828);
		else if(curStage == 'land-destroyed')
			timeBar.createFilledBar(0xFF000000, 0xFFCC4915);
		else if(curStage == 'land-bloody')
			timeBar.createFilledBar(0xFF000000, 0xFF640505);
		else if(curStage == 'land-bloody-day')
			timeBar.createFilledBar(0xFF000000, 0xFFE4B0B0);
		else if(curStage == 'land-lol')
			timeBar.createFilledBar(0xFF000000, 0xFF5DD43F);
		...
		healthBarBG.xAdd = -4;
		healthBarBG.yAdd = -4.25;
		if(ClientPrefs.downScroll) healthBarBG.y = 0.11 * FlxG.height;

		healthBarBG2 = new AttachedSprite('creepyUI/healthBar');
		...
		healthBarBG2.xAdd = -6.8;
		healthBarBG2.yAdd = -7.5;
	...
	function cacheCountdown()
	{
		...
		introAssets.set('creepy', ['creepyUI/ready', 'creepyUI/set', 'creepyUI/go']);

		var introAlts:Array<String> = introAssets.get('default');
		if (isCreepyStage) introAlts = introAssets.get('creepy');
	...
	{
		scoreTxt.text = 'Score: ' + songScore + ' | Health: ' + Math.round(health * 50) + '% | Misses: ' + songMisses + ' | Accuracy: ' + '${Highscore.floorDecimal(ratingPercent * 100, 2)}% $ratingFC';
	...
	function openScaredEditor()
	{
		persistentUpdate = false;
		paused = true;
		cancelMusicFadeTween();
		MusicBeatState.switchState(new NotCheatState());
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Flash Light':
				...
				{
					camOther.flash(FlxColor.WHITE, 1, null, false);
				}
			...
			case 'Camera Tween Zoom':
				...
				if(duration == 0) {
					...
					camTween = FlxTween.tween(camGame, {zoom: targetZoom}, duration, {
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['toastie', 'ur_bad', 'ur_good', 'hype', 'fliqpy_nomiss', 'hell_beat']);
	...
	private function cachePopUpScore()
	{
		...
		if (isCreepyStage)
		{
			creepyShitPart1 = 'creepyUI/';
		}

		Paths.image(creepyShitPart1 + "sick");
		Paths.image(creepyShitPart1 + "good");
		Paths.image(creepyShitPart1 + "bad");
		Paths.image(creepyShitPart1 + "shit");
		Paths.image(creepyShitPart1 + "combo");
		
		for (i in 0...10) {
			Paths.image(creepyShitPart1 + 'num' + i);
		}
	}
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		{
			if(CoolUtil.difficultyString() == 'HELL')
				health -= 0.015;
			else
				health -= 0.03;
		}
		...
		rating.loadGraphic(Paths.image(creepyShitPart1 + daRating.image));
		...
		var comboSpr:FlxSprite = new FlxSprite().loadGraphic(Paths.image(creepyShitPart1 + 'combo'));
		...
		{
			var numScore:FlxSprite = new FlxSprite().loadGraphic(Paths.image(creepyShitPart1 + 'num' + Std.int(i)));
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		{
			if(dad.curCharacter == 'psycho-fliqpy') {
				if(health > 0.1) {
					updateScore(true);
					health -= 0.0295;
				}
			}
			if(dad.curCharacter == 'fliqpy-crazy') {
				if(health > 0.2) {
					updateScore(true);
					health -= 0.0175;
				}
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if(boyfriend.curCharacter == 'bf-scared-fire')
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				switch(achievementName)
				{
					...
					case 'hell_beat':
						if(isStoryMode && CoolUtil.difficultyString() == 'HELL'
							&& storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
						{
							unlock = true;
						}
