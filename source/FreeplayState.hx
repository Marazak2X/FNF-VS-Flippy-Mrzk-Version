...
class FreeplayState extends MusicBeatState
{
	...
	function changeSelection(change:Int = 0, playSound:Bool = true)
	{
		...
		if (curSelected == 0 || curSelected == 1 || curSelected == 4)
		{
			chromeValue = 0.0;
		}
	...
	override function beatHit()
	{
		...
		if (['flippy-roll', 'happy-tree-land', 'massacre', 'flippin out', 'unflipped out', 'traumatized'].contains(songs[curSelected].songName.toLowerCase()))
		{
			...
			if (songs[curSelected].songName.toLowerCase() == 'flippin out')
			{
			...
			else if(songs[curSelected].songName.toLowerCase() == 'massacre')
			{
				if(ClientPrefs.shaders)
				{
			...
			else if(songs[curSelected].songName.toLowerCase() == 'traumatized')
			{
				if(ClientPrefs.shaders)
				{
					chromeValue += FlxG.random.float(0.02, 0.0865);
